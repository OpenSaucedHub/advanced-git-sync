...
if [ "$NEEDS_AUTH" = true ]; then
    ...
    case $choice in
        1)
            ...
            echo "  Requesting scopes: repo, workflow, write:packages, read:packages, admin:ssh_signing_key, read:org, admin:org"
            unset GITHUB_TOKEN
            gh auth login --hostname github.com --web --git-protocol https --scopes "repo,workflow,write:packages,read:packages,admin:ssh_signing_key,read:org,admin:org"
            echo "✓ Authentication complete"
            ;;
        2)
            echo "Please provide your Personal Access Token for $GIT_USER"
            echo "You can create one at: https://github.com/settings/tokens"
            echo "Required scopes: repo, workflow, write:packages, read:packages, admin:ssh_signing_key, read:org, admin:org"
